...
import { TopicCategory, TOPIC_CATEGORIES, TopicStatus, TOPIC_STATUSES } from "@/types/clanGuild.types";
...
interface TopicsToolbarProps {
  ...
  onCategoryChange: (c: TopicCategory | "All") => void;
...
export function TopicsToolbar({
  ...
  const CATEGORY_OPTIONS = ["All", ...TOPIC_CATEGORIES];
  const STATUS_OPTIONS = ["All", ...TOPIC_STATUSES];
    ...
    <div className="flex flex-col lg:flex-row gap-3 lg:items-center lg:justify-between">
      ...
      <div className="flex flex-wrap gap-2">
        <FilterDropdown
          ...
          value={category}
          onChange={(v: string) => onCategoryChange(v as TopicCategory | "All")}
        />
        ...
        <FilterDropdown
          ...
          value={status}
          onChange={(v: string) => onStatusChange(v as TopicStatus | "All")}
        />
      </div>
    </div>
  );
